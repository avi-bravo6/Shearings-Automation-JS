...
test('Test Financial Security Page', async ({ page }) => {
    ...
    await test.step('Navigate to Financial Security page', async () => {
    ...
    await test.step('Wait for table to be attached to the DOM', async () => {
    ...
    await test.step('Wait for rows to load', async () => {
    ...
    await test.step('Validate rows against expected data', async () => {
        const rows = page.locator('xpath=/html/body/section[2]/div/section/div/div/div/table/tbody/tr');
        const rowCount = await rows.count();
        ...
        for (let i = 0; i < expectedData.length; i++) {
            ...
            try {
                ...
                console.warn(`Row ${i + 1} validation failed: ${error.message}`);
