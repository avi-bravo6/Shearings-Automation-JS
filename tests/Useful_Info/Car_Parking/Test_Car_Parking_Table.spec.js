...
test('Test Car Parking Table', async ({ page }) => {
    ...
    await allure.step('Click on Car Parking button', async () => {
    ...
    await allure.step('Verify navigation to the Car Parking page', async () => {
    ...
    await allure.step('Wait for the table to be visible', async () => {
    ...
    await allure.step('Get all rows from the table and validate the row count', async () => {
        const tableLocator = page.locator('xpath=/html/body/section[3]/div/div/div/table');
        ...
        if (rowCount === 0) {
            allure.addAttachment('Car Parking Table', 'No rows found');
            throw new Error('Table is visible but contains no rows');
        }

        allure.addAttachment('Car Parking Table Rows', `Row count: ${rowCount}`);
    });
    ...
    await allure.step('Validate each row in the table against expected data', async () => {
        const tableLocator = page.locator('xpath=/html/body/section[3]/div/div/div/table');
        const rows = tableLocator.locator('tbody tr');
        const rowCount = await rows.count();
